...
class LanGuideMedSegWrapper(pl.LightningModule):
    def __init__(self, args):
        ...
        module_name, class_name = args.model_name.rsplit('.', 1)
        model_module = importlib.import_module(module_name)
        model_class = getattr(model_module, class_name)
        self.model = model_class() 

        self.lr = args.lr
        self.epoch = args.max_epochs
        self.history = {}
    ...
    def shared_step(self,batch,batch_idx):
        output = self(batch)
        preds, gt, generated, generated_gt, feat_w, feat_s, text_embed, reconstructed_text = output

        feat_w = F.normalize(feat_w, dim=1)
        feat_s = F.normalize(feat_s, dim=1)
        IAC_loss = 1 - F.cosine_similarity(feat_w, feat_s.detach(), dim=1).mean()

        cycle_loss = nn.L1Loss()(reconstructed_text, text_embed.detach())

        loss = self.loss_fn(preds, gt) + 0.1 * nn.L1Loss()(generated, generated_gt) + 0.05 * IAC_loss + 0.07 * cycle_loss
        return {'loss': loss, 'preds': preds.detach(), 'y': gt.detach()}
